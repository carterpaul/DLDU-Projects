...
for epoch in range(epochs):  # train the model
  ...
  # select batch_size random indeces within size of xss
  batch_indeces = random.sample(range(xss.size()[0]), batch_size)

  # set batch_xss and batch_yss to the values of xss and yss at those indeces
  batch_xss = torch.stack([xss[i] for i in batch_indeces])
  batch_yss = torch.stack([yss[i] for i in batch_indeces])
